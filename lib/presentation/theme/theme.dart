import 'package:flutter/material.dart';
import 'package:google_fonts/google_fonts.dart';
import 'package:weight_tracker/presentation/router/route_transition.dart';

ThemeData theme() {
  return ThemeData(
    primarySwatch: Colors.green,
    primaryColorLight: Color(0xFF66C577),
    primaryColor: Color(0xFF40AA52),
    primaryColorDark: Color(0xFF239837),
    accentColor: Color(0xFFF55D3E),
    scaffoldBackgroundColor: Colors.white,
    backgroundColor: Color(0xFFF0F0F0),
    buttonColor: Color(0xFF40AA52),
    textTheme: TextTheme(
      headline1: GoogleFonts.lobsterTwo(
          fontSize: 77, fontWeight: FontWeight.w300, letterSpacing: -1.5),
      headline2: GoogleFonts.lobsterTwo(
          fontSize: 48, fontWeight: FontWeight.w300, letterSpacing: -0.5),
      headline3:
          GoogleFonts.lobsterTwo(fontSize: 39, fontWeight: FontWeight.w400),
      headline4: GoogleFonts.lobsterTwo(
          fontSize: 27, fontWeight: FontWeight.w400, letterSpacing: 0.25),
      headline5:
          GoogleFonts.lobsterTwo(fontSize: 19, fontWeight: FontWeight.w400),
      headline6: GoogleFonts.lobsterTwo(
          fontSize: 16, fontWeight: FontWeight.w500, letterSpacing: 0.15),
      subtitle1: GoogleFonts.lobsterTwo(
          fontSize: 13, fontWeight: FontWeight.w400, letterSpacing: 0.15),
      subtitle2: GoogleFonts.lobsterTwo(
          fontSize: 11, fontWeight: FontWeight.w500, letterSpacing: 0.1),
      bodyText1: GoogleFonts.openSans(
        fontSize: 13,
        fontWeight: FontWeight.w400,
        // letterSpacing: 0.5,
      ),
      bodyText2: GoogleFonts.openSans(
        fontSize: 12,
        fontWeight: FontWeight.w400,
        // letterSpacing: 0.25,
      ),
      button: GoogleFonts.openSans(
        fontSize: 12,
        fontWeight: FontWeight.w500,
        // letterSpacing: 1.25,
      ),
      caption: GoogleFonts.openSans(
        fontSize: 10,
        fontWeight: FontWeight.w400,
        // letterSpacing: 0.4,
      ),
      overline: GoogleFonts.openSans(
        fontSize: 8,
        fontWeight: FontWeight.w400,
        // letterSpacing: 1.5,
      ),
    ),
    pageTransitionsTheme: PageTransitionsTheme(
      builders: {
        TargetPlatform.android: CustomPageTransitionBuilder(),
        TargetPlatform.iOS: CustomPageTransitionBuilder(),
      },
    ),
  );
}
